...
st.markdown("Make sure enter numbers as precise as possible. For example, a price range of 28.1 - 39.8 can give a very different prediction than 28.1224 - 39.8142.")
...
with c1:
    st.subheader('Data')
    st.markdown("Ideally, we want to work with a dataset that has only\n1. positions that never had liquidity partially removed,\n2. positions that never had additional liquidity added,\n3. positions that ran its course and had 100% liquidity removed and no new liquidity added afterwards.\nA dataset of 1 and 2 was downloaded from Revert Finance on Nov 23, 2022 at 23:08:00 UTC. The data-pulling script was written by [0x1egolas](https://twitter.com/0x1egolas). Unfortunately, data of 3 cannot be easily obtained.")
    st.markdown("We then took a subset of data that meet all of the following criteria:\n - initial deposit value is between \$50 and \$1M US Dollars,\n- age of position is at least 3 hours,\n- lower BTCETH limit is at least 4,\n- upper BTCETH limit is at most 400.")
    st.markdown("Ranges of Important Variables on this subset:\n- Age ranges from 0.137 to 131.5 days,\n- Deposit ranges from \$52.1 to \$281,625 USD,\n- Price lower limit ranges from 5.1 to 41.3,\n- Price upper limit ranges from 23.2 to 364.9,\n- ROI ranges from -22.45% to 28.23%,\n- Fee APR ranges from 3.16% to 2608.56%.")
    st.markdown("Finally, we split this subset into a training set (80%, 273 records) and a test set (20%, 69 records). We trained ML models on the training set with cross-validation (cv). Throughout the training, we did not touch the test set, nor did we once peek at model performance on the test set. We chose the best model according to cv error. Only after we finalized the model, we used it on the test set to get the predictions displayed at the top of this page.")
with c2:
    ...
    st.markdown("Model selection:\n- XGBOOST regressor was trained, where a single parameter, learning rate, was tuned for optimal performance via 5-fold cross-validation, with negative mean absolute error as the scoring metric.\n- The best model for ROI prediction had a CV error of 3.5%.\n- The best model for Fee APR prediction had a CV error of 21.3%, which isn't big given the fee apr in the sample ranges from 3.16% to 2608.56%.")
    st.markdown("Test Set Performance:\n- The best model for ROI prediction had a test error of 3%.\n- The best model for Fee APR prediction had a test error of 22.9%.")
